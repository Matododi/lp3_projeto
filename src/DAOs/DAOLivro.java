...
public class DAOLivro extends DAOGenerico<Livro> {
    ...
    public List<Livro> listByNome(String nome) {
        return em.createQuery("SELECT e FROM Livro e WHERE e.preco LIKE :nome").setParameter("nome", "%" + nome + "%").getResultList();
    }
    ...
    public List<Livro> listInOrderNome() {
        return em.createQuery("SELECT e FROM Livro e ORDER BY e.preco").getResultList();
    }
    ...
    public List<String> listInOrderNomeStrings(String qualOrdem) {
        ...
        for (int i = 0; i < lf.size(); i++) {
            ls.add(lf.get(i).getIdLivro() + "-" + lf.get(i).getPreco());
        }
        return ls;
    }
public static void main(String[] args) {
        ...
        for (Livro livro : listaLivro) {
            System.out.println(livro.getIdLivro() + "-" + livro.getPreco());
        }
    }}
